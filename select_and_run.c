 ...
 * By Ivan Gavryushin (dcc0@mail.ru, ivangavr777@gmail.com, mol0t@list.ru). MSK. Butovo. 2025*/
	...
	//====ЕСЛИ СДЕЛАН ВЫБОР===
	  ...
	  if (select > 0) {
		...
		printf("Для справки  добавьте ключ -h: select_and_run.comb -h\n");
		printf("О программе: select_and_run.comb -a\n");
	...
	//Проверим файл при старте
		file  = fopen("data.txt", "r");
		if (file == NULL) {
		printf("Нет файла  data.txt  Для справки  добавьте ключ -h: select_and_run.comb -h\n");
		return 1;
		}
		else {
		fclose(file);
	...
	printf("В директории с программой должен быть файл data.txt. В нем 29 опций выбора программ с абсолютными путями. Пример: /bin/ls. 30  опция - выход\n");
	return 0;
	}
	...
	system("clear");
	sleep(1);
	printf("Running Mol0T Commander. Ver. 1.0. Written in C(89). By Ivan Gavryushin (ivangavr777@gmail.com, mol0t@list.ru, dcc0@yandex.ru). MSK. Butovo. 2025\n");
	sleep(5);
	printf("Для справки  добавьте ключ -h: select_and_run.comb -h\n");
	sleep(5);
	printf("Спасибо!\n");
